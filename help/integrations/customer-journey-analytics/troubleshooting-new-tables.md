...
| I campi Adobe Advertising richiesti non sono presenti nello schema | Verificare che i campi obbligatori di Adobe Advertising siano presenti nello schema in `_experience.adcloud.conversionDetails` (vedere la tabella di riferimento dei campi seguente).<br><br>Se uno dei due campi non è presente, verificare che il gruppo di campi **Estensione completa Adobe Advertising Cloud ExperienceEvent** sia stato salvato nello schema, quindi aggiornare l&#39;editor dello schema. |
| L’URL della pagina di destinazione non include i parametri di query richiesti | Accertati che l’URL della pagina di destinazione includa i parametri di query necessari. In un click-through di un annuncio, l&#39;URL della pagina di destinazione deve contenere entrambi i parametri di query, ad esempio `https://www.example.com/landing-page?s_kwcid=AL!12345!3!abc123&ef_id=abc123xyz:G:s` (consulta la tabella di riferimento di seguito per le cause probabili). |
...
**Riferimento: campi schema obbligatori**
...
**Riferimento: parametri di query mancanti**
