...
| Non vengono effettuate richieste di interazione o raccolta a edge.adobedc.net. | Vedi &quot;[Problemi di installazione e configurazione](#issues-installation-setup)&quot; > &quot;[L&#39;estensione WebSDK non viene inizializzata](#websdk-extension-doesn't-initialize)&quot; |
...
| Le conversioni vengono confermate in uno strumento di debugger, ma non vengono visualizzate nei rapporti di Adobe Advertising. | Consulta la sezione &quot;[Problemi di configurazione dell&#39;estensione Advertising](#advertising-extension-setup-issues)&quot; |
| L’ID visitatore cambia da una pagina all’altra. | Consulta la sezione &quot;[Problemi di identità ed ECID](#identity-and-ecid-issues)&quot; |
...
| Le modifiche apportate in [!DNL Tags] non vengono applicate al sito attivo. | Consulta la sezione &quot;[Problemi di compilazione e pubblicazione della libreria](#library-build-and-publishing-issues)&quot; |
...
* Non vengono effettuate richieste di interazione o raccolta a edge.adobedc.net.

#### Possibili cause e verifica/risoluzione

| Causa | Correzione |
| ----- | --- |
| Libreria non pubblicata o in stato di bozza | Vai a [Flusso di pubblicazione](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/publishing-flow) e assicurati che la libreria che contiene l&#39;estensione WebSDK sia nello stato approvato/pubblicato. |
| Codice di incorporamento mancante o ambiente errato | Verificare che il [[!DNL Tags] codice di incorporamento](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/environments/environments) nella pagina Web faccia riferimento all&#39;ambiente corretto (Dev/Stage/Prod). Cercare l&#39;ambiente nel tag `<head>` per il tag di script `//assets.adobedtm.com/...`. |
| Conflitto tra carico asincrono e carico sincrono | Assicurarsi che sia presente un solo codice di incorporamento [[!DNL Tags] &#x200B;](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/environments/environments) per pagina Web. I codici di incorporamento duplicati causano race condition. |
| Blocco di Content Security Policy (CSP) | Aggiungi `edge.adobedc.net` `and assets.adobedtm.com` ai [CSP `connect-src` e `script-src` direttive](https://experienceleague.adobe.com/it/docs/experience-platform/collection/use-cases/configuring-a-csp). |
...
* Nei rapporti di Adobe Analytics o Adobe Advertising non viene visualizzato alcun dato.
...
| I servizi stream di dati non sono abilitati per la proprietà tag. | [Aprire le impostazioni dello stream di dati](https://experienceleague.adobe.com/it/docs/experience-platform/datastreams/configure) e verificare che i servizi seguenti siano abilitati:<ul><li>Adobe Advertising (per conversione/sincronizzazione pubblico)</li><li>Adobe Experience Platform (per l’acquisizione del profilo)</li></ul> |
| Sandbox non corrispondente | Assicurati che lo stream di dati appartenga alla stessa [sandbox di Adobe Experience Platform](https://experienceleague.adobe.com/it/docs/experience-platform/sandbox/home) dello schema e del set di dati. Un errore comune è la creazione di un flusso di dati nella sandbox di produzione, ma il fatto di puntare gli schemi alla sandbox di sviluppo. |
...
| In Experience Platform non è disponibile alcuna area eventi view-through dopo l’attivazione della chiamata di interazione view-through. | Verificare che un inserzionista sia configurato e abilitato nella sezione [[!UICONTROL Advertiser] della configurazione dell&#39;estensione WebSDK](https://experienceleague.adobe.com/it/docs/experience-platform/tags/extensions/client/web-sdk/configure/advertising). Se l&#39;inserzionista è stato immesso manualmente, selezionare nuovamente l&#39;inserzionista dal menu a discesa [!UICONTROL Advertiser]. Dopo aver configurato l&#39;inserzionista, rigenera e ripubblica la libreria. |
...
* L’ID visitatore cambia da una pagina all’altra.
...
| Causa | Correzione |
| ----- | --- |
| I cookie di terze parti sono bloccati. | Esegui la migrazione alla raccolta dati CNAME di prime parti [configurando un ID cookie di prime parti nella configurazione Edge Network del flusso di dati](https://experienceleague.adobe.com/it/docs/experience-platform/datastreams/configure). |
...
* La regola viene salvata e inclusa nella [build della libreria](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/builds) attiva.
...
* Le condizioni della regola non sono troppo restrittive. Effettua il test [rimuovendo temporaneamente le condizioni](https://experienceleague.adobe.com/it/docs/experience-platform/tags/ui/rules) per isolare il problema.
* Ordine delle regole corretto. Se più regole condividono lo stesso evento, controlla l&#39;[ordinamento regole](https://experienceleague.adobe.com/it/docs/experience-platform/tags/ui/rules).
...
* Le modifiche apportate in [!DNL Tags] non vengono applicate al sito attivo.
...
| Causa | Correzione |
| ----- | --- |
| Le modifiche non sono state aggiunte a una libreria. | Nel flusso di lavoro di pubblicazione, confermare che le modifiche sono state aggiunte a una [libreria](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/libraries) nell&#39;ambiente di sviluppo. Se [apporti modifiche alla libreria](https://experienceleague.adobe.com/it/docs/experience-platform/tags/publish/libraries#manage-library-changes), aggiungi le risorse, quindi salva e genera la libreria. |
...
| Il codice di incorporamento è per l’ambiente sbagliato. | Verifica che il codice di incorporamento nella pagina sia per l’ambiente corretto. Ad esempio, per testare il comportamento di produzione, verifica che il codice da incorporare sia il codice da incorporare di produzione. |
| Compilazione della libreria non riuscita in modo invisibile all&#39;utente. | Nel flusso di lavoro di pubblicazione, verifica se la libreria mostra uno stato [!UICONTROL Build Failed]. Apri la libreria e rivedi il registro di build; le cause comuni degli errori di build sono configurazioni di regole non valide o conflitti di versioni delle estensioni. |
