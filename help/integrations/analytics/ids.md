...
*Applicabile alle DSP pubblicitarie e[!DNL Advertising Search]*
...
* Una voce view-through viene acquisita quando un utente visita il sito dopo aver visualizzato un annuncio ma non lo fa clic. [!DNL Analytics] registra un view-through se sono soddisfatte due condizioni:
   * Il visitatore non dispone di click-through per un [!DNL DSP] o [!DNL Search] annuncio durante [finestra di lookback su clic](#lookback-a4adc).
...
### Formato AMO ID per [!DNL Search]

ID AMO per [!DNL Search] segui un formato distinto per ogni motore di ricerca. Il formato per tutti i motori di ricerca inizia con quanto segue:
...
Di seguito sono riportati i formati AMO ID completi per un paio di reti di annunci. Per i formati AMO ID per altre reti di annunci, contatta il tuo [!DNL Adobe] team di account.
