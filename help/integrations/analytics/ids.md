...
*Applicabile alla pubblicità DSP e[!DNL Advertising Search]*
...
* Una voce view-through viene acquisita quando un utente visita il sito dopo aver visualizzato un annuncio ma non facendo clic su di esso. [!DNL Analytics] registra una visualizzazione view-through se sono soddisfatte due condizioni:
   * Il visitatore non ha click-through per un [!DNL DSP] o [!DNL Search] annuncio durante [fai clic sull’intervallo di lookback](#lookback-a4adc).
...
AMO ID per [!DNL Search] segui un formato distinto per ogni motore di ricerca. Il formato di tutti i motori di ricerca inizia con quanto segue:
