---
title: (Nuova interfaccia) Eliminare i bulksheet caricati e i file di errore
description: Scopri come eliminare i file bulksheet e i file di errore nella nuova interfaccia utente di Search, Social e Commerce.
feature: Search Bulksheets
product_v2: id: a829a185-511f-4bf8-8dcf-9e684f8011cf
feature_v2: id: aed5e38a-3e62-42fa-8d16-cd080729b2a0
subfeature_v2: id: e58024d1-d6da-420c-80af-6be211808316id: f3d33161-c519-436e-bbbd-730ba428736b
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
source-git-commit: 739034010787c2016720bef37fb75dc8efbae58b
workflow-type: tm+mt
source-wordcount: 147
ht-degree: 0%

---

# (Nuova interfaccia) Eliminare i bulksheet caricati e i file di errore

Puoi eliminare manualmente i file di bulksheet, i file di errore di convalida della pagina di destinazione e altri file di errore. I file vengono eliminati automaticamente 30 giorni dopo il caricamento o la generazione.

>[!TIP]
>
>Se si è registrato un bulksheet ma sono stati ricevuti errori, non eliminare il file di errore fino alla risoluzione di tutti gli errori.

1. Nel menu principale, fare clic su **[!UICONTROL Setup]** \> **[!UICONTROL Bulksheets]**.

1. Selezionare la casella di controllo accanto a ogni file da eliminare.

1. Nella barra degli strumenti Azioni in blocco fare clic su **[!UICONTROL Delete]**.

1. Nel messaggio di conferma, fare clic su **[!UICONTROL Confirm]** per confermare l&#39;eliminazione.

   Se è programmata la pubblicazione di uno dei bulksheet selezionati, viene inviata una notifica prima della conferma. L&#39;eliminazione di un bulksheet pianificato annulla il post in sospeso.

>[!MORELIKETHIS]
>
>* [(Nuova interfaccia) Informazioni sulla gestione dei dati della campagna tramite bulksheet](about.md)
>* [(Nuova interfaccia) Interruzione di un processo bulksheet in corso](stop-job.md)
