---
title: Gestione dei modelli di feed
description: Scopri come creare un modello di annuncio per i feed di dati di inventario.
...
Puoi creare modelli di annunci specifici per i motori di ricerca tramite i quali elaborare i dati di inventario. Creare modelli separati per annunci di testo espansi/estesi e annunci di ricerca responsive [!DNL Google Ads] annunci commerciali, e [!DNL Microsoft Advertising] acquisti pubblicitari.

Oltre a creare modelli da zero, è possibile creare nuovi modelli duplicando quelli esistenti.

Dopo aver creato o clonato un modello e averlo associato a un file di feed dati, è possibile propagare i dati nel file tramite il modello per creare i dati della campagna e la struttura dell’account.
   ...
   1. (Solo testo e annunci di ricerca responsive) Fai clic sul pulsante **[!UICONTROL Ads]** ed eseguire una delle operazioni seguenti:
      ...
      * Per aggiungere una variante di annuncio, effettua le seguenti operazioni:
         ...
         1. (Facoltativo) Per aggiungere un altro set di copie di annunci a un annuncio, che può essere utilizzato se una qualsiasi delle righe nella copia dell’annuncio originale supera la lunghezza massima dopo che tutti i parametri dinamici sono stati sostituiti con i dati durante la propagazione, fai clic su **[!UICONTROL Add Alternate]** e quindi aggiungere i valori alternativi.
            ...
            >* Solo i campi della copia dell’annuncio che superano la lunghezza massima vengono sostituiti con il valore alternativo. Ad esempio, se solo un titolo o titolo originale è troppo lungo, la variante di annuncio generata utilizza il titolo o il titolo alternativo e le descrizioni originali. Assicurati pertanto che la copia dell’annuncio alternativa sia significativa se combinata con la copia dell’annuncio originale.
