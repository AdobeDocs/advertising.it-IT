---
title: Sicurezza del brand e qualità dei media
...
Il nostro team di sorveglianza antifrode collabora a stretto contatto con partner leader del settore, come [!DNL Interactive Advertising Bureau], [!DNL Trust and Accountability Group] [!DNL (TAG)] e [!DNL WhiteOps], per curare attentamente l&#39;inventario sulla nostra piattaforma. Grazie alla gestione proattiva della nostra offerta, DSP garantisce che tutti gli inserzionisti della piattaforma siano protetti dal traffico non umano (bot, crawler, traffico da data center e frodi) e che la distribuzione avvenga solo in contesti sicuri per il marchio.
...
DSP ha creato potenti strumenti e sistemi interni per la gestione delle frodi nella piattaforma EMC, collaborando con i principali fornitori del settore come [!DNL Whiteops] e [!DNL Integral Ad Science].

Inoltre, Adobe lavora a stretto contatto con [!DNL IAB] e [!DNL TAG] per garantire un blocco delle frodi solido e conforme agli standard di settore per proteggere i nostri inserzionisti, sfruttando strumenti come [!DNL ads.txt] (vedi la sezione precedente), l&#39;elenco di bot e spider [!DNL IAB] e l&#39;elenco IP del data center [!DNL TAG].
...
### Mappatura, suddivisione in livelli e categorizzazione dell&#39;inventario

La mappatura dell’inventario è il processo dettagliato di revisione e onboarding necessario per tutti i nuovi inventari prima che vengano aggiunti alla nostra piattaforma. Questo processo è stato progettato per garantire la sicurezza e la qualità di tutto l&#39;inventario su DSP.
...
DSP fornisce sia un elenco di siti bloccati a livello globale che l&#39;opzione per la creazione di elenchi di siti bloccati personalizzati per gli inserzionisti e gli account.

#### Elenco siti bloccati a livello globale da DSP {#global-blocked-sites}

DSP mantiene un elenco di siti bloccati a livello globale di siti ritenuti non sicuri per l’esecuzione di annunci. Questo elenco contiene siti con contenuti sgradevoli (come odio o terrore) e siti infetti da bot, pre-roll falso, domini non corrispondenti e altre attività fraudolente.

Come parte della nostra iniziativa Brand Safety per eliminare le attività che frodano gli inserzionisti, tutti i siti vengono controllati utilizzando le misure nell’elenco dei siti bloccati del grafico. Tutti i siti che non superano i controlli di sicurezza del marchio vengono aggiunti all’elenco dei siti bloccati a livello globale. Poiché DSP gestisce questo elenco in modo dinamico, i siti possono spostarsi da un elenco all’altro in qualsiasi momento, in base all’analisi più recente sulla sicurezza del marchio.
...
### Targeting degli argomenti

Il targeting degli argomenti di DSP consente di eseguire il targeting o bloccare gli elenchi di parole chiave sfruttando il nostro partner contestuale leader del settore [!DNL Comscore]. Il targeting degli argomenti ti aiuta a garantire che gli annunci vengano sempre serviti in un ambiente in linea con il tuo marchio, sia che si tratti di bloccare contenuti dannosi o di garantire la spesa in un contesto che garantisce un risultato maggiore.
...
DSP ha stretto una partnership con [!DNL DoubleVerify] per offrire la sua soluzione di targeting [!DNL Authentic Brand Suitability], che consente di creare un set centralizzato di requisiti di idoneità al brand per eseguire il targeting su tutte le piattaforme di acquisto per coerenza.

Dopo aver creato un segmento [!DNL DoubleVerify] con il targeting necessario, puoi utilizzarlo all&#39;interno di DSP per replicare le regole del blocco post-offerta con pre-offerta tra gli ambienti web.

È possibile specificare un ID segmento [!DNL DoubleVerify] per ogni inserzionista e quindi, facoltativamente, [abilitare o disabilitare [!UICONTROL Authentic Brand Suitability] per ogni posizionamento](/help/dsp/campaign-management/placements/placement-settings.md). DSP fattura il tuo account per l’utilizzo dell’ID segmento.

Per ulteriori informazioni sulle funzionalità, contatta direttamente [!DNL DoubleVerify] o contatta il tuo account team Adobe.
