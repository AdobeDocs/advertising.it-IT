...
I messaggi di errore per gli invii di annunci non riusciti possono provenire da Advertising DSP o da [!DNL FreeWheel]. Trovare i messaggi di errore nella colonna [!UICONTROL API Response] nella finestra di dialogo [[!UICONTROL FreeWheel Status]](freewheel-check-status.md).
...
| [!DNL Invalid external_deal_id:] \&lt;deal_id\> | L’ID offerta inviato non esiste o non è attivo all’estremità di Adobe. | Assicurati che l’offerta sia attiva, quindi invia nuovamente l’annuncio. |
...
| [!DNL Ad with identifier] \&lt;*nome annuncio*\> [!DNL was not found.] | La chiave dell’annuncio inviata non esiste o non è attiva nell’estremità di Adobe. | Trova la chiave dell’annuncio corretta, quindi invia nuovamente l’annuncio. |
...
| 404 | Non trovato | La risorsa richiesta non è disponibile. Se l&#39;ID Creative non viene trovato nell&#39;operazione PUT, viene restituito un 404. | Contatta il team del tuo account Adobe. |
| 405 | Metodo non consentito | È stata effettuata una richiesta di una risorsa utilizzando un metodo di richiesta non supportato da tale risorsa (ad esempio, utilizzando GET su un metodo che richiede l&#39;invio di dati tramite POST o PUT su una risorsa di sola lettura). | Contatta il team del tuo account Adobe. |
...
| DATA_DSP_GET_FAILURE | 422 | DSP non esiste nel sistema. |
